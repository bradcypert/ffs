...
class Scheduler {
    ...
    static createSchedule() {
        const rooms = lodash.values(this.getRooms());
    ...
    static getCreepsInRoom(room) {
        return lodash.values(Game.creeps).filter((c) => c.room.name === room.name);
    }
    static assignTaskByBodyParts(creep) {
        const counts = lodash.reduce(creep.body, (acc, val) => {
